...
export async function POST(request, { params }) {
  try {
    ...
    if (!job.company) {
      // Try to find the company ID from the job's company reference
      const jobWithCompany = await Job.findById(id).populate("company")
      ...
      job.company = jobWithCompany.company
    }
    ...
    const applicationToCreate = {
      job: jobId,
      user: userObjectId,
      company: job.company._id || job.company, // Ensure we get the ID if it's a populated object
      fullName: applicationDataFromForm.fullName,
      email: applicationDataFromForm.email,
      phone: applicationDataFromForm.phone,
      status: "pending",
      aiAnalysis: applicationDataFromForm.aiAnalysis,
      cosine:applicationDataFromForm.cosine
    }
